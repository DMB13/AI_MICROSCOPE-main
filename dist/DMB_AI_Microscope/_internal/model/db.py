...
class Database:
    ...
    def get_all_records(self) -> List[Dict[str, Any]]:
        ...
        sql = "SELECT * FROM clinical_records ORDER BY timestamp DESC"
        with self._lock:
    ...
    def get_records_by_date_range(self, start_date: str, end_date: str) -> List[Dict[str, Any]]:
        """Return records within a date range (inclusive). Dates should be 'YYYY-MM-DD' format."""
        sql = """SELECT * FROM clinical_records 
                 WHERE date(timestamp) BETWEEN ? AND ? 
                 ORDER BY timestamp DESC"""
        with self._lock:
            cur = self.conn.execute(sql, (start_date, end_date))
            rows = [dict(row) for row in cur.fetchall()]
            cur.close()
        return rows
