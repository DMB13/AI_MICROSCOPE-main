...
# 3. Install dependencies (TensorFlow 2.18.1 with Keras 3.x)
...
**A:** 
- **Operating System**: Ubuntu 18.04+ or Windows 10/11
- **Python**: 3.11.0 or higher (tested with 3.11.9)
- **RAM**: Minimum 8GB, recommended 16GB+ for 1.9GB model
- **Storage**: 10GB free space for model and dependencies
- **Camera**: USB or integrated camera
- **Display**: 1024x768 minimum resolution
- **TensorFlow**: 2.18.1 with Keras 3.x included

### Q: Does it work on Windows/Mac?
**A:** ✅ Yes! The system is optimized for both Ubuntu Linux and Windows 10/11 with TensorFlow 2.18.1. Mac support may be added in future versions.
...
**A:** The model (EfficientNetV2M Clinical Rugged) has been trained on 34 bacterial species with high accuracy for common pathogens. However:
...
**A:** The system can identify 34 species including:
- **Common Pathogens**: E. coli, S. aureus, K. pneumoniae
- **Hospital Acquired**: P. aeruginosa, A. baumannii
- **Other Species**: Full list available in `model/class_indices.json`
- **Note**: Species name is only displayed when confidence ≥ 70%
...
**A:** 
- **Input**: Automatically resized to 224x224 for AI processing (EfficientNetV2M)
...
- **Clinical**: 0.9 (90%) for clinical acceptance
- **Display**: Species name hidden below 0.7 (70%)
- **Alerts**: Low confidence results are highlighted in red
...
**A:** 
- **Database**: `clinical_records.db` (SQLite, root directory)
...
**A:** Model issues with TensorFlow 2.18.1:
1. **Check Model File**: Verify `model/best_clinical_rugged_1777619657.keras` exists (~1.2GB)
2. **File Permissions**: Ensure read access to model file
3. **TensorFlow Version**: Ensure TF 2.18.1 with Keras 3.x is installed
4. **Memory**: Check if system has sufficient RAM (8GB minimum, 16GB recommended)
5. **Compatibility**: System automatically handles tensor mismatch issues
6. **Status**: ✅ Advanced compatibility fixes implemented
...
- **Access Control**: Role-based authentication (admin, technician, lab_manager)
- **Encryption**: Database encryption support
- **Audit Trail**: All actions logged with medical fields
- **Login Required**: Authentication required on startup
